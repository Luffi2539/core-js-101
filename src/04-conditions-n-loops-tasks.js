...
function getDigitalRoot(num) {
  const sum = num.toString().split('').reduce((prev, cur) => prev + Number(cur), 0);
  if (sum > 9) return sum.toString().split('').reduce((prev, cur) => prev + Number(cur), 0);
  return sum;
}
